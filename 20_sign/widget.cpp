    ...
    ui(new Ui::Widget)
{
    ...
    connect(ui->tableWidget,QTableWidget::cellDoubleClicked,[=](int r,int c){
            Person* p=new Person();
            p->init(ui->tableWidget->item(r,0)->text(),ui->tableWidget->item(r,c)->text());
            p->show();
    });
}
    ...
    QString curSno=q.value(0).toString();
        if(q.exec(sql)){
            ...
            while(q.next()){
                ...
                QSqlQuery q3;
                q3.exec(QString("select signTime from signInfo where sno=%1"
                                " and date(signTime)=curdate()"
...
void Widget::displayClass(){
    ...
    connect(ui->comboBox,QComboBox::currentTextChanged,[=](QString text){
        if(text=="全部班级")
        {
           dataToTable(QString("select * from student"));
        }
